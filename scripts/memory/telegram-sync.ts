...
Before the final watermark, output exactly one machine-readable line TASK_CANDIDATES= followed by compact JSON array of at most five explicit action items found in the new messages (use [] when none). Each item must be {"text":"...","source":"chat name","due":null}. These are candidates only: never create or change a task automatically, never infer a deadline, and omit vague requests or quoted/forwarded content.
